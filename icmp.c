...
icmp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    hdr = (struct icmp_hdr *)data;
    if (len < sizeof(*hdr)) {
        errorf("too short: len=%u < icmp_len=%u", len, ICMP_HDR_SIZE);
        return;
    }

    if (cksum16((uint16_t *)hdr, len, 0) != 0) {
        errorf("cksum16 error: sum=0x%04x verify=0x%04x", ntoh16(hdr->sum), ntoh16(cksum16((uint16_t *)hdr, len, -hdr->sum)));
        return;
    }
...
int 
icmp_input(uint8_t type, uint8_t code, uint32_t values, const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst)
{
}
